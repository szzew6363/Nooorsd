...
const MODES: { id: UltraMode; label: string; sub?: string; model: string; color: string }[] = [
  ...
  { id: "ultracode", label: "ultracode", sub: "workflows+agent", model: "opus4.5 / o3", color: "#7c3aed" },
];
    ...
    return (
      <div key={node.path}>
        <div className="flex items-center gap-1 px-2 py-[3px] cursor-pointer rounded group"
          style={{ paddingLeft:8+depth*12, background:isSel?"rgba(124,58,237,0.12)":undefined }}
          ...
          {node.type==="dir"
            ...
            : <><span style={{color}}>{getFileIcon(node.ext)}</span>
                ...
                <button onClick={e=>{e.stopPropagation();addCtx(node);}} className="opacity-0 group-hover:opacity-100 transition-opacity ml-1 shrink-0" style={{ color:isAdd?"#4ade80":"#7c3aed" }}>
        ...
        <div className="border-t" style={{ borderColor:"rgba(124,58,237,0.15)" }}>
          <div className="flex items-center gap-1 px-2 py-1" style={{ background:"#0e0e14" }}>
            ...
            <button onClick={() => onAddToContext({ name:fileContent.path.split("/").pop()!, content:fileContent.content, updatedAt:Date.now() })}
              className="text-[8px] font-bold px-1.5 py-0.5 rounded shrink-0" style={{ background:"rgba(124,58,237,0.15)", color:"#a78bfa", border:"1px solid rgba(124,58,237,0.3)" }}>
              +ctx
            </button>
          </div>
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const term = new Terminal({
        theme: { background:"#0c0c10", foreground:"#d1d5db", cursor:"#7c3aed", cursorAccent:"#0c0c10", selectionBackground:"rgba(124,58,237,0.3)", black:"#1e1e2e", red:"#f38ba8", green:"#a6e3a1", yellow:"#f9e2af", blue:"#89b4fa", magenta:"#cba6f7", cyan:"#89dceb", white:"#cdd6f4" },
    ...
    <div className="flex-1 flex overflow-hidden min-h-0">
      ...
      <div className="w-56 flex flex-col border-r shrink-0" style={{ borderColor:"rgba(255,255,255,0.06)", background:"#0a0a0d" }}>
        <div className="flex items-center gap-2 px-3 py-2 border-b" style={{ borderColor:"rgba(255,255,255,0.05)" }}>
          ...
          <span className="text-[10px] font-mono font-bold" style={{ color:"#a78bfa" }}>{status?.branch??"-"}</span>
          ...
          {loading?<div className="text-center py-4 text-[10px]" style={{color:"#333"}}>Loading...</div>
            ...
            :status?.files.map(f => (
              <div key={f.path} onClick={() => { setSelected(f.path); loadDiff(f.path); }}
                ...
                style={{ background:selected===f.path?"rgba(124,58,237,0.12)":undefined }}
          ...
          <div className="overflow-y-auto" style={{ maxHeight:100 }}>
            {status?.log.slice(0,6).map(l => (
              <div key={l.hash} className="px-3 py-0.5">
                <span className="text-[8px] font-mono" style={{ color:"#7c3aed" }}>{l.hash}</span>
      ...
      <div className="flex-1 overflow-auto" style={{ background:"#080808" }}>
        {!selected ? (
          <div className="flex flex-col items-center justify-center h-full gap-3" style={{ color:"#2a2240" }}>
...
function LogBlockView({ block, onToggleThinking }: { block: LogBlock; onToggleThinking: (id:number)=>void }) {
  ...
  if (block.kind==="tool") return (
    <div className="flex items-start gap-2 my-1 px-2 py-1.5 rounded-lg" style={{ background:"rgba(124,58,237,0.06)", border:"1px solid rgba(124,58,237,0.15)" }}>
      <Cpu className="w-3 h-3 mt-0.5 shrink-0" style={{ color:"#7c3aed" }}/>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-[9px] font-mono font-bold" style={{ color:"#6d28d9" }}>{block.tool}</span>
          <span className="text-[9px] font-mono truncate" style={{ color:"#5b21b6" }}>{block.arg}</span>
  ...
  if (block.kind==="workflow") return (
    <div className="flex items-center gap-2 px-2 py-1 rounded my-1 text-[10px] font-mono" style={{ background:"rgba(124,58,237,0.08)", border:"1px solid rgba(124,58,237,0.2)" }}>
      <Zap className="w-3 h-3 shrink-0" style={{ color:"#7c3aed" }}/><span style={{ color:"#a78bfa" }}>{block.text}</span>
    </div>
  );
  ...
  if (block.kind==="text") return (
    <div className="relative group my-1">
      <div className="pl-3 py-1.5 text-[12px] font-mono leading-relaxed" style={{ color:"#d1d5db", borderLeft:"2px solid rgba(124,58,237,0.3)", whiteSpace:"pre-wrap", wordBreak:"break-word" }}>
        {block.text}
        {block.streaming&&<span className="inline-block w-1.5 h-3.5 ml-0.5 align-middle animate-pulse rounded-sm" style={{ background:"#7c3aed" }}/>}
      </div>
...
function AgentStepIndicator({ step, max }: { step: number; max: number }) {
  return (
    <div className="flex items-center gap-2 px-3 py-1 shrink-0" style={{ background:"rgba(124,58,237,0.08)", borderBottom:"1px solid rgba(124,58,237,0.15)" }}>
      <div className="flex gap-0.5">
        {Array.from({length:max}).map((_,i) => (
          <div key={i} className="w-1.5 h-1.5 rounded-full transition-all" style={{ background:i<step?"#7c3aed":i===step?"#a78bfa animate-pulse":"rgba(255,255,255,0.1)" }}/>
        ))}
      </div>
      <span className="text-[9px] font-mono" style={{ color:"#a78bfa" }}>Agent Step {step}/{max}</span>
      <span className="text-[8px] font-mono ml-auto animate-pulse" style={{ color:"#7c3aed" }}>⬤ executing tools</span>
    </div>
  );
}
      ...
      {open&&(
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
          ...
          <motion.div initial={{scale:0.96,y:16}} animate={{scale:1,y:0}} exit={{scale:0.96,y:16}} transition={{duration:0.18}}
            ...
            style={{ background:"#0c0c10", border:"1px solid rgba(124,58,237,0.25)", boxShadow:"0 0 80px rgba(124,58,237,0.2),0 30px 60px rgba(0,0,0,0.9)", height:"min(90vh,800px)" }}>
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 border-b shrink-0 select-none" style={{ borderColor:"rgba(255,255,255,0.06)", background:"#121218" }}>
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={()=>setAgentLoop(p=>!p)} className="flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-bold transition-all"
                  style={{ background:agentLoop?"rgba(124,58,237,0.2)":"rgba(255,255,255,0.05)", border:`1px solid ${agentLoop?"rgba(124,58,237,0.5)":"rgba(255,255,255,0.1)"}`, color:agentLoop?"#a78bfa":"#444" }}
                  title="Toggle Agent Loop">
                  <Cpu className="w-2.5 h-2.5"/> {agentLoop?"AGENT":"MANUAL"}
                </button>
                <button onClick={()=>setShowFileTree(!showFileTree)} className="p-1 rounded" style={{ color:showFileTree?"#7c3aed":"#444" }} title="File Tree">
                  <FolderOpen className="w-3.5 h-3.5"/>
                </button>
                <button onClick={()=>setShowFiles(!showFiles)} className="p-1 rounded" style={{ color:showFiles?"#7c3aed":"#444" }} title="Upload Files">
                  <FileText className="w-3.5 h-3.5"/>
                </button>
                <button onClick={()=>setShowSettings(!showSettings)} className="p-1 rounded" style={{ color:showSettings?"#7c3aed":"#444" }}>
            ...
            <AnimatePresence>
              {showSettings&&(
                <motion.div initial={{height:0}} animate={{height:"auto"}} exit={{height:0}} className="overflow-hidden shrink-0" style={{ borderBottom:"1px solid rgba(124,58,237,0.15)", background:"#0e0e14" }}>
                  <div className="px-4 py-3 flex items-center gap-3 flex-wrap">
                    ...
                    <button onClick={()=>{ const k=newKeyInput.trim(); if(!k) return; const dp=detectProvider(k); localStorage.setItem(LS_KEY_APIKEY,k); localStorage.setItem(LS_KEY_PROVIDER,dp); setApiKey(k); setProvider(dp); setNewKeyInput(""); setShowSettings(false); }}
                      className="px-3 py-1.5 rounded-lg text-[10px] font-bold shrink-0" style={{ background:"rgba(124,58,237,0.2)", border:"1px solid rgba(124,58,237,0.4)", color:"#a78bfa" }}>حفظ</button>
            ...
            <div className="flex items-center border-b shrink-0" style={{ borderColor:"rgba(255,255,255,0.06)", background:"#0d0d12" }}>
              {TABS.map(tab => (
                <button key={tab.id} onClick={()=>setActiveTab(tab.id)}
                  ...
                  style={{ borderBottom:activeTab===tab.id?`2px solid ${currentMode.color}`:"2px solid transparent", color:activeTab===tab.id?currentMode.color:"#444", background:activeTab===tab.id?"rgba(124,58,237,0.05)":undefined }}>
                  {tab.icon}{tab.label}
                </button>
              ))}
            </div>
            ...
            {activeTab==="chat" && (
              <>
                ...
                <div className="flex items-center gap-2 px-4 py-1 shrink-0" style={{ background:"#0e0e14", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                  ...
                  <span className="text-[8px] font-mono px-1.5 py-0.5 rounded" style={{ background:"rgba(124,58,237,0.08)", color:"#5b21b6", border:"1px solid rgba(109,40,217,0.15)" }}>
                      ...
                      <div className="border-b shrink-0" style={{ borderColor:"rgba(255,255,255,0.06)", background:"#0e0e14" }}>
                        <div className="flex items-center justify-between px-3 py-1.5">
                          <div className="flex items-center gap-1.5">
                            ...
                            {(activeSession?.files??[]).length>0&&<span className="text-[8px] px-1 rounded font-mono" style={{ background:"rgba(124,58,237,0.15)", color:"#7c3aed" }}>{(activeSession?.files??[]).length}</span>}
                          </div>
                        ...
                        {(activeSession?.files??[]).length>0&&(
                          <div className="flex flex-wrap gap-1 px-3 pb-2">
                            {(activeSession?.files??[]).map(f => (
                              <div key={f.name} className="flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono group" style={{ background:"rgba(124,58,237,0.08)", border:"1px solid rgba(124,58,237,0.2)", color:"#a78bfa" }}>
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-3 font-mono min-h-0" style={{ background:"#0c0c10" }}>
                    {logs.length===0&&(
                      <div className="flex flex-col items-center justify-center h-full gap-4 pb-8">
                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background:"rgba(124,58,237,0.08)", border:"1px solid rgba(124,58,237,0.2)" }}>
                    ...
                    <div className="flex items-center gap-1.5 mb-1.5 flex-shrink-0">
                      {running?(
                        ...
                        <button onClick={execute} disabled={!input.trim()} className="flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-bold disabled:opacity-30 transition-all"
                          style={{ background:currentMode.id==="ultracode"?"rgba(124,58,237,0.2)":"rgba(139,92,246,0.12)", border:`1px solid ${currentMode.color}40`, color:currentMode.color }}>
                ...
                {/* Mode Slider */}
                <div className="shrink-0 relative overflow-hidden" style={{ background:"linear-gradient(135deg,#1e1030 0%,#150d2a 40%,#1a0a3d 70%,#0f0720 100%)", borderTop:"1px solid rgba(139,92,246,0.15)" }}>
                  <div className="absolute inset-0 pointer-events-none" style={{ background:`radial-gradient(ellipse 50% 100% at ${(modeIdx/(MODES.length-1))*100}% 50%,rgba(124,58,237,0.15) 0%,transparent 70%)` }}/>
                  <div className="relative px-6 py-3">
                    <div className="relative flex items-center mt-1">
                      <div className="absolute top-[5px] left-0 right-0 h-0.5" style={{ background:"rgba(255,255,255,0.06)" }}/>
                      <div className="absolute top-[5px] left-0 h-0.5 transition-all duration-400" style={{ width:`${(modeIdx/(MODES.length-1))*100}%`, background:`linear-gradient(to right,rgba(139,92,246,0.4),${currentMode.color})` }}/>
                      {MODES.map((m,idx) => {
                        ...
                        return (
                          <div key={m.id} className="flex-1 flex flex-col items-center relative">
                            <button onClick={()=>!running&&setMode(m.id)} disabled={running} className="flex flex-col items-center gap-0.5">
                              <div className="relative z-10 rounded-full transition-all duration-300" style={{ width:isActive?12:7, height:isActive?12:7, background:isActive?m.color:isPast?"rgba(139,92,246,0.45)":"rgba(255,255,255,0.1)", boxShadow:isActive?`0 0 10px ${m.color},0 0 20px ${m.color}60`:undefined }}/>
                              <span className="text-[8px] font-mono font-bold mt-1" style={{ color:isActive?m.color:"#3a2e55" }}>{m.label}</span>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-[8px] font-mono" style={{ color:"#3a2e55" }}>
                        {running?<span className="animate-pulse" style={{ color:"#7c3aed" }}>{currentMode.id==="ultracode"?"Ultracode running...":currentMode.id+" running..."}</span>:currentMode.model}
                      </span>
